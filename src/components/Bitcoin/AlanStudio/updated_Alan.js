...
intent(
  ...
  async (p) => {
    ...
    if (period === 'month')
        ...
        .subtract(4, `${period}s`)
        .format(DATE_FORMAT)
    else if (period === 'year') end = api.moment().format(DATE_FORMAT)

    const url = `https://api.coindesk.com/v1/bpi/historical/close.json?currency=USD&start=${start}&end=${end}`
    //         const url = `https://api.coindesk.com/v1/bpi/historical/close.json?currency=USD&start=2020-10-08&end=2021-10-08`;
    //         https://api.coindesk.com/v1/bpi/historical/close.json?currency=USD&start=2022-10-08&end=2022-11-08
    ...
    p.play({ command: 'pricePeriod', url })
    p.play(
...
intent(
  ...
  async (p) => {
    const currencyCode =
      p.CURRENCY && p.CURRENCY.value ? getCurrencyCode(p.CURRENCY.value) : 'USD'
...
function getStartDate(period, timeZone) {
  console.log('Given period: ' + period)
  if (period === 'year')
    return api
      .moment()
      .tz(timeZone)
    ...
    console.log(
      'Given Month Period: ' +
        api.moment().tz(timeZone).subtract(4, `${period}s`).format(DATE_FORMAT),
    )
    return api
      .moment()
      .tz(timeZone)
